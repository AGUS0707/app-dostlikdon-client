    ...
    return (
        <AdminLayout history={props.history}>
            <button type="button" className="btn btn-success d-block ml-auto"
                    onClick={() => props.updateState({open: !props.open})}>Add Menus
            </button>
                ...
                {props.menus.map((item, index) => {
                    return (
                        <tr>
                            ...
                            <td className="d-flex"><button type="button" className="btn btn-primary" onClick={() => {props.updateState({open: true, selectedItem: item, url: item.url, submenu: item.submenu})}}>Edit</button>
                            <button type="button" className="btn btn-danger ml-2" onClick={() => props.updateState({deleteModal: !props.deleteModal, selectedIndex: item.id})}>Delete</button></td>
                        </tr>
                    )
                })}
                </tbody>
